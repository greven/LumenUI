...
local function updatePosition(self, p, anchor, rP, x, y, xOffset, yOffset)
  if not x then
    if C.db.profile.movers[self:GetName()].point then
      p, anchor, rP, x, y = unpack(C.db.profile.movers[self:GetName()].point)
...
local function mover_SavePosition(self, p, anchor, rP, x, y)
  C.db.profile.movers[self:GetName()].point = {p, anchor, rP, x, y}
end

local function mover_ResetPosition(self)
  if not self.isSimple and InCombatLockdown() then
    return
  end
  ...
  C.db.profile.movers[self:GetName()].point = nil
...
local function mover_WasMoved(self)
  local dest = C.db.profile.movers[self:GetName()]
...
local function mover_Enable(self)
  ...
  if enabledMovers[name] or not disabledMovers[name] then
    return
  end
...
local function mover_Disable(self)
  ...
  if disabledMovers[name] or not enabledMovers[name] then
    return
  end
...
function P.Movers:CleanUpConfig()
  C.db.profile.movers = E:DiffTable(defaults, C.db.profile.movers)
end

function P.Movers:UpdateConfig()
  E:UpdateTable(defaults, C.db.profile.movers)
