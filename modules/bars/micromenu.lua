...
local ALERTS = {"CharacterMicroButton", "CollectionsMicroButton", "EJMicroButton", "GuildMicroButton",
                "TalentMicroButton"}
...
local function getTooltipPoint(self)
    ...
    local p, ap, x, y = "TOPLEFT", "BOTTOMRIGHT", 2, -2

    if quadrant == "BOTTOMLEFT" or quadrant == "BOTTOM" then
        p, ap, x, y = "BOTTOMLEFT", "TOPRIGHT", 2, 2
    elseif quadrant == "TOPRIGHT" or quadrant == "RIGHT" then
        p, ap, x, y = "TOPRIGHT", "BOTTOMLEFT", -2, -2
    elseif quadrant == "BOTTOMRIGHT" then
        p, ap, x, y = "BOTTOMRIGHT", "TOPLEFT", -2, 2
    end
...
local function updatePushedTexture(button)
    button:SetPushedTexture("Interface\\AddOns\\LumenUI\\media\\textures\\micromenu")
...
local function updateHighlightTexture(button)
    button:SetHighlightTexture("Interface\\AddOns\\LumenUI\\media\\textures\\micromenu", "ADD")
...
local function button_OnEnter(self)
    local p, ap, x, y = getTooltipPoint(self._parent)

    GameTooltip:SetOwner(self, "ANCHOR_NONE")
    GameTooltip:SetPoint(p, self, ap, x, y)
...
local function button_GetAnchor(self)
    return BARS:GetBar(self._config.parent) or BARS:GetBar("micromenu1")
end
...
local function handleMicroButton(button)
    ...
    border:SetTexture(C.db.global.buttons.border.texture or M.textures.border)
    border:SetSize(C.db.global.buttons.border.size)
    border:SetOffset(C.db.global.buttons.border.offset)
    button.Border = border

    local flash = button.Flash
    flash:SetTexture("Interface\\AddOns\\LumenUI\\media\\textures\\micromenu")
    flash:SetTexCoord(unpack(TEXTURE_COORDS.PUSHED))
    ...
    icon:SetTexture("Interface\\AddOns\\LumenUI\\media\\textures\\micromenu")
    ...
    local function characterButton_OnEnter(self)
        ...
        if self:IsEnabled() then
            ...
            for i = 1, 17 do
                ...
                if cur then
                    GameTooltip:AddDoubleLine(slots[i], ("%d%%"):format(cur), 1, 1, 1,
                        E:GetGradientAsRGB(cur / 100, C.db.global.colors.ryg))
                end
            end
...
-- Inventory
local inventoryButton_OnClick, inventoryButton_OnEvent, inventoryButton_Update, inventoryButton_UpdateConfig,
    inventoryButton_UpdateIndicator, inventoryButton_UpdateSlots
                    ...
                    if info.maxQuantity and info.maxQuantity > 0 then
                        if info.quantity == info.maxQuantity then
                            GameTooltip:AddDoubleLine(info.name,
                                CURRENCY_DETAILED_TEMPLATE:format(BreakUpLargeNumbers(info.quantity),
                                    BreakUpLargeNumbers(info.maxQuantity), info.iconFileID), 1, 1, 1,
                                E:GetRGB(C.db.global.colors.red))
                        else
                            GameTooltip:AddDoubleLine(info.name,
                                CURRENCY_DETAILED_TEMPLATE:format(BreakUpLargeNumbers(info.quantity),
                                    BreakUpLargeNumbers(info.maxQuantity), info.iconFileID), 1, 1, 1,
                                E:GetRGB(C.db.global.colors.green))
                        end
                    else
                        GameTooltip:AddDoubleLine(info.name, CURRENCY_TEMPLATE:format(
                            BreakUpLargeNumbers(info.quantity), info.iconFileID), 1, 1, 1, 1, 1, 1)
                    end
                end
            end
...
do
    ...
    local function bag_OnClick(self, button)
        if button == "RightButton" then
            ...
            local p, ap, x, y = getTooltipPoint(LumBagBar)

            LumBagBar.FilterMenu:SetAnchor(p, self, ap, x * 5, y * 5)
            LumBagBar.FilterMenu:ClearLines()
            LumBagBar.FilterMenu:AddLines(generateMenuLinesForBag(self))
            LumBagBar.FilterMenu:Toggle()
    ...
    local function updateCTARewards(dungeonID, dungeonName)
        if IsLFGDungeonJoinable(dungeonID) then
            for shortageIndex = 1, LFG_ROLE_NUM_SHORTAGE_TYPES do
                local eligible, forTank, forHealer, forDamager, numRewards =
                    GetLFGRoleShortageRewards(dungeonID, shortageIndex)
            ...
            for _, role in next, roles do
                ...
                for _, v in next, cta[role] do
                    if v then
                        ...
                        for i = 1, #v do
                            GameTooltip:AddDoubleLine(v[i].name, v[i].quantity .. v[i].texture, gray.r, gray.g, gray.b,
        ...
        if self:IsEnabled() then
            ...
            for i = 1, savedInstances + savedWorldBosses do
                if i <= savedInstances then
                    instanceName, _, instanceReset, _, _, _, _, _, _, difficultyName, numEncounters, encounterProgress =
                    ...
                    if instanceReset > 0 then
                        ...
                        GameTooltip:AddDoubleLine(instanceName, encounterProgress .. " / " .. numEncounters, 1, 1, 1,
                            color.r, color.g, color.b)
                        GameTooltip:AddDoubleLine(difficultyName, SecondsToTime(instanceReset, true, nil, 3), gray.r,
                            gray.g, gray.b, gray.r, gray.g, gray.b)
                    end
                else
                    ...
                    if instanceReset > 0 then
                        ...
                        GameTooltip:AddDoubleLine(L["WORLD_BOSS"], SecondsToTime(instanceReset, true, nil, 3), gray.r,
                            gray.g, gray.b, gray.r, gray.g, gray.b)
                    end
                end
            end
...
do
    ...
    local function mainMenuButton_OnEnter(self)
        ...
        if self:IsEnabled() then
            ...
            GameTooltip:AddDoubleLine(L["LATENCY_HOME"],
                LATENCY_TEMPLATE:format(E:GetGradientAsHex(latencyHome / PERFORMANCEBAR_MEDIUM_LATENCY,
                                            C.db.global.colors.gyr), latencyHome), 1, 1, 1)
            GameTooltip:AddDoubleLine(L["LATENCY_WORLD"],
                LATENCY_TEMPLATE:format(E:GetGradientAsHex(latencyWorld / PERFORMANCEBAR_MEDIUM_LATENCY,
                                            C.db.global.colors.gyr), latencyWorld), 1, 1, 1)
            ...
            if IsShiftKeyDown() then
                ...
                for i = 1, #addOns do
                    if addOns[i][3] then
                        ...
                        GameTooltip:AddDoubleLine(addOns[i][1], MEMORY_TEMPLATE:format(m / 1024), 1, 1, 1,
                            E:GetGradientAsRGB(m / (memUsage == m and 1 or (memUsage - m)), C.db.global.colors.gyr))
                    end
                end
    ...
    function mainMenuButton_UpdateIndicator(self)
        ...
        self.Indicator:SetVertexColor(E:GetGradientAsRGB(latencyWorld / PERFORMANCEBAR_MEDIUM_LATENCY,
                                          C.db.global.colors.gyr))
    end
end
...
local function moveMicroButtons()
    if isInit then
        ...
        local bar = BARS:GetBar("micromenu1")
        bar:UpdateButtonList()
        bar:UpdateLayout()

        bar = BARS:GetBar("micromenu2")
        bar:UpdateButtonList()
        bar:UpdateLayout()
    end
end
...
local function updateMicroButtons()
    if isInit then
        for _, button in next, BARS:GetBar("micromenu1")._buttons do
            if button._config.enabled then
                button:Show()
            end
        end

        for _, button in next, BARS:GetBar("micromenu2")._buttons do
            if button._config.enabled then
                button:Show()
            end
        end
    ...
    if not isInit then
        ...
        for id, data in next, BUTTONS do
            ...
            elseif id == "inventory" then
                ...
                local slots = CreateFrame("Frame", "LumBagBar", UIParent)
                slots:Hide()
                ...
                local menu = LibDropDown:NewMenu(slots, "LumBagFilterMenu")
                menu:SetStyle("MENU")
                ...
                slots:SetPoint(point.p, point.anchor, point.ap, point.x, point.y)
        ...
        bar1:SetPoint(point.p, point.anchor, point.ap, point.x, point.y)
...
function BARS:UpdateMicroMenu()
    ...
    self:GetBar("micromenu1"):UpdateVisibility()
    self:GetBar("micromenu2"):UpdateVisibility()
end

function BARS:ForMicroButton(id, method, ...)
