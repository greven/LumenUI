...
local sharedEvents = {"PLAYER_TALENT_UPDATE"}
...
local events = {
    ...
    power_cur = "UNIT_POWER_FREQUENT UNIT_POWER_UPDATE UNIT_MAXPOWER UNIT_CONNECTION PLAYER_FLAGS_CHANGED PLAYER_DEAD PLAYER_ALIVE",
...
local customTags = {
    ...
    color = function(_, _, color)
        ...
        return "|c" .. E:ToHex(C.db.global.colors[color])
    end,
    ...
    color_difficulty = function(unit)
        ...
        return "|c" .. E:ToHex(C.db.global.colors.difficulty.impossible)
    end,
    ...
    health_cur = function(unit, _, colorCap, colorStatus)
        ...
        if colorStatus then
            offline = "|c" .. E:ToHex(C.db.global.colors.light_gray) .. offline .. "|r"
            dead = "|c" .. E:ToHex(C.db.global.colors.light_red) .. dead .. "|r"
        end
    ...
    npc_type = function(unit, realUnit, color)
        ...
        if classification == "worldboss" or UnitLevel(realUnit or unit) <= 0 then
            return color and "|c" .. E:ToHex(C.db.global.colors.difficulty.impossible) .. "Boss|r" or "Boss"
        elseif classification == "rare" then
            return color and "|cff008FF7Rare|r" or "Rare"
        elseif classification == "rareelite" then
            return color and "|cff008FF7Rare|r |c" .. E:ToHex(C.db.global.colors.difficulty.very_difficult) .. "Elite|r" or
                "Rare Elite"
        elseif classification == "elite" then
            return color and "|c" .. E:ToHex(C.db.global.colors.difficulty.difficult) .. "Elite|r" or "Elite"
        end
    ...
    npc_type_short = function(unit, realUnit, color)
        ...
        if classification == "worldboss" or UnitLevel(realUnit or unit) <= 0 then
            return color and "|c" .. E:ToHex(C.db.global.colors.difficulty.impossible) .. "+|r" or "+"
        elseif classification == "rare" then
            return color and "|cff008FF7R|r" or "R"
        elseif classification == "rareelite" then
            return color and "|cff008FF7R|r|c" .. E:ToHex(C.db.global.colors.difficulty.very_difficult) .. "+|r" or "R+"
        elseif classification == "elite" then
            return color and "|c" .. E:ToHex(C.db.global.colors.difficulty.difficult) .. "+|r" or "+"
