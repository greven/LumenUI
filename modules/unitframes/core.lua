...
function UF:For(unit, method, ...)
    if units[unit] and C.db.profile.modules.unitframes.units[unit].enabled then
        if unit == "party" then
...
function UF:Create(unit, name)
    if not units[unit] then
        ...
        elseif unit == "party" then
            ...
            E:SetPosition(header, C.db.profile.modules.unitframes.units[unit].point)
            objects[unit] = header
        else
            local object
            ...
            E:SetPosition(object, C.db.profile.modules.unitframes.units[unit].point)
            objects[unit] = object
        end
...
function UF:Init()
    if not isInit and C.db.profile.modules.unitframes.enabled then
        self:UpdateColors()
        UF:CreateFrames()
